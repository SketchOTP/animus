...
Per-tool lines in ``project_history.md`` (under the project workspace) are
appended when the client sends ``hermes_project_path`` (absolute directory)
in the JSON body for ``/v1/chat/completions`` and ``/v1/responses``, or when
``HERMES_PROJECT_PATH`` is set in the gateway environment. The path must
resolve to an existing directory under the gateway user's home directory.
On each such request the gateway runs ``ensure_workspace_files`` so
``project_history.md`` / ``repo_map.md`` exist (repo map is generated when
missing).
...
def _resolve_hermes_project_path_for_history(body: Optional[Dict[str, Any]] = None) -> Optional[Path]:
    """Resolve ``hermes_project_path`` from JSON body or ``HERMES_PROJECT_PATH`` env.

    Must be an existing directory under ``$HOME`` (after resolve) so API clients
    cannot redirect logging to arbitrary locations.
    """
    raw: Optional[str] = None
    if body is not None:
        v = body.get("hermes_project_path")
        if isinstance(v, str) and v.strip():
            raw = v.strip()
    if raw is None:
        ev = (os.environ.get("HERMES_PROJECT_PATH") or "").strip()
        if ev:
            raw = ev
    if not raw:
        return None
    try:
        ...
        logger.warning("hermes_project_path invalid: %r", raw)
        return None
    if not p.is_dir():
        logger.warning("hermes_project_path not a directory: %s", p)
        return None
    try:
        p.relative_to(Path.home().resolve())
    except ValueError:
        logger.warning("hermes_project_path outside $HOME; rejecting: %s", p)
        return None
    return p


def _ensure_hermes_project_workspace(root: Optional[Path]) -> None:
    """Create ``project_history.md`` / ``repo_map.md`` under *root* when missing."""
    if root is None:
        return
    try:
        ...
        logger.debug("ensure_workspace_files failed for %s", root, exc_info=True)


def _tool_complete_chain_project_history(
    project_root: Optional[Path],
    inner: Optional[Callable[..., None]],
) -> Optional[Callable[..., None]]:
    """Chain an optional inner ``tool_complete_callback`` with project history logging."""

    if project_root is None and inner is None:
        return None

    def _cb(tool_call_id: Any, function_name: Any, function_args: Any, function_result: Any) -> None:
        if inner is not None:
            try:
                inner(tool_call_id, function_name, function_args, function_result)
            except Exception:
                logger.debug("tool_complete inner callback failed", exc_info=True)
        if project_root is not None:
            try:
                from agent.project_workspace import append_gateway_tool_history_line

                append_gateway_tool_history_line(
                    project_root,
                ...
                logger.debug("project_history tool append skipped: %s", exc)
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        # Hermes Chat and ops probes compare this to the Chat server's HERMES_HOME + cron path.
        try:
            from hermes_constants import get_hermes_home

            hh = get_hermes_home()
            payload["hermes_home"] = str(hh)
            payload["hermes_home_resolved"] = str(hh.resolve())
        except Exception as exc:
            payload["hermes_home"] = None
            payload["hermes_home_resolved"] = None
            payload["hermes_home_error"] = str(exc)
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        _tool_hist_cb = _tool_complete_chain_project_history(project_hist_root, None)
        ...
        if stream:
            ...
            agent_task = asyncio.ensure_future(self._run_agent(
                ...
                tool_complete_callback=_tool_hist_cb,
                agent_ref=agent_ref,
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                """
                nonlocal emitted_assistant_text
            ...
            try:
                ...
                logger.warning(
                    "chat_sse: agent_task raised after queue drain: %s",
                    exc,
                    exc_info=True,
                )
                result = {"final_response": "", "error": str(exc)}

            # Subprocess shims (cursor-agent, copilot-acp) and some failure modes can
            # finish the turn with a non-empty ``final_response`` but never enqueue
            # token deltas.  Hermes Chat (and other OpenAI SSE clients) would show an
            # empty bubble — synthesize one ``delta.content`` chunk from the outcome.
            if not emitted_assistant_text:
                fallback = ""
                if isinstance(result, dict):
                    fallback = (result.get("final_response") or "").strip()
                    if not fallback:
                        err_val = result.get("error")
                        if isinstance(err_val, str):
                            fallback = err_val.strip()
                        elif err_val is not None:
                            fallback = str(err_val).strip()
                if not fallback:
                    fallback = (
                        "No assistant text was streamed for this completion. "
                        "Check hermes-gateway logs; for Cursor CLI verify "
                        "`cursor-agent` on PATH and `CURSOR_API_KEY` (or logged-in CLI)."
                    )
                last_activity = await _emit(fallback)
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            _responses_tool_complete = _tool_complete_chain_project_history(
                project_hist_root,
                _on_tool_complete,
            )
        ...
        _responses_tool_hist_only = _tool_complete_chain_project_history(project_hist_root, None)
