...
from agent.task_complexity import estimate_task_complexity as _estimate_task_complexity
...
_DEFAULT_DELEGATE_ACP_COMMAND = "claude"
_DEFAULT_CLAUDE_ACP_ARGS = ["--acp", "--stdio"]
_DEFAULT_CLAUDE_COMPLEXITY_MODEL_MAP = {
    "low": "haiku",
    "medium": "sonnet",
    "high": "opus",
}
...
def _acp_command_basename(command: Optional[str]) -> str:
    if not command:
        return ""
    return Path(str(command)).name.lower()


def _is_claude_acp_command(command: Optional[str]) -> bool:
    base = _acp_command_basename(command)
    return bool(base) and base.startswith("claude")


def _acp_args_include_model(args: Optional[List[str]]) -> bool:
    if not args:
        return False
    normalized = [str(a).strip() for a in args if str(a).strip()]
    for i, tok in enumerate(normalized):
        if tok in {"--model", "-m"}:
            return True
        if tok.startswith("--model="):
            return True
        if tok == "-m" and i + 1 < len(normalized):
            return True
    return False


def _complexity_model_map_from_cfg(cfg: dict) -> Dict[str, str]:
    mapping = dict(_DEFAULT_CLAUDE_COMPLEXITY_MODEL_MAP)
    raw = cfg.get("acp_complexity_model_map")
    if isinstance(raw, dict):
        for level in ("low", "medium", "high"):
            val = raw.get(level)
            if isinstance(val, str) and val.strip():
                mapping[level] = val.strip()
    return mapping


def _resolve_delegate_acp_transport(
    *,
    cfg: dict,
    goal: Optional[str],
    ...
    task_count: int,
    override_command: Optional[str],
    override_args: Optional[List[str]],
    parent_command: Optional[str],
    parent_args: Optional[List[str]],
) -> tuple[Optional[str], List[str]]:
    """Resolve ACP command/args with Claude-default + complexity auto-modeling."""
    configured_default = str(cfg.get("default_acp_command") or "").strip()
    effective_command = (
        override_command
        or parent_command
        or configured_default
        or _DEFAULT_DELEGATE_ACP_COMMAND
    )

    effective_args = list(
        override_args if override_args is not None else (parent_args or [])
    )

    if not _is_claude_acp_command(effective_command):
        return effective_command, effective_args

    for required in _DEFAULT_CLAUDE_ACP_ARGS:
        if required not in effective_args:
            effective_args.append(required)

    auto_select = cfg.get("acp_auto_model_selection", True)
    if not is_truthy_value(auto_select, default=True):
        return effective_command, effective_args

    if _acp_args_include_model(effective_args):
        return effective_command, effective_args
    ...
    model_map = _complexity_model_map_from_cfg(cfg)
    selected_model = model_map.get(complexity) or model_map.get("medium")
    if selected_model:
        effective_args.extend(["--model", selected_model])
...
def _build_child_agent(
    ...
    effective_acp_command, effective_acp_args = _resolve_delegate_acp_transport(
        cfg=delegation_cfg,
        goal=goal,
        context=context,
        toolsets=toolsets,
        task_count=task_count,
        override_command=override_acp_command,
        override_args=override_acp_args,
        parent_command=getattr(parent_agent, "acp_command", None),
        parent_args=getattr(parent_agent, "acp_args", []) or [],
    )
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "If omitted, delegation defaults to Claude Code via 'claude'. "
            ...
            "acp_args": {
                ...
                "items": {"type": "string"},
                "description": (
                    "Arguments for the ACP command. For Claude defaults, Hermes ensures ['--acp', '--stdio'] "
                    "and auto-selects a Claude model by task complexity unless a model flag is already provided. "
                    "Example explicit override: ['--acp', '--stdio', '--model', 'opus']."
                ),
            },
        },
        "required": [],
