...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "cursor-agent":
        try:
            from agent.cursor_agent_client import _resolve_executable_and_base_args

            exe, args = _resolve_executable_and_base_args()
        except Exception:
            exe, args = "cursor-agent", []
        base_url = os.getenv(pconfig.base_url_env_var or "", "").strip() if pconfig.base_url_env_var else ""
        if not base_url:
            base_url = pconfig.inference_base_url
        ...
        return {
            ...
            "command": exe,
            "args": args,
            "resolved_command": resolved if ok else None,
            "base_url": base_url,
            "logged_in": ok,
        }
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "cursor-agent":
        ...
        exe, args = _resolve_executable_and_base_args()
        if "/" in exe or "\\" in exe:
            resolved_command = exe if Path(exe).is_file() else None
        else:
            resolved_command = shutil.which(exe)
        if not resolved_command:
            raise AuthError(
                f"Could not find the Cursor CLI ({exe!r}). Install Cursor or set "
                "HERMES_CURSOR_AGENT_COMMAND to the full path of `cursor-agent` or `cursor`.",
                provider=provider_id,
