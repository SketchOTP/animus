...
def auth_add_command(args) -> None:
    ...
    _p_meta = PROVIDER_REGISTRY.get(provider)
    if _p_meta and _p_meta.auth_type == "external_process":
        print(
            f"{provider} uses a local CLI subprocess, not `hermes auth add`.\n"
            "Run:  hermes model\n"
            f"  → pick provider \"{provider}\" and a model (e.g. auto)."
        )
        raise SystemExit(0)
