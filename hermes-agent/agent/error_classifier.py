...
def classify_api_error(
    ...
    *,
    ...
    # Anthropic "extra usage" (Claude Max add-on meter) — two shapes:
    #   • 429 + "extra usage" + "long context" — 1M tier requires the add-on.
    #   • 400/429 + "out of extra usage" or claude.ai/settings/usage — meter
    #     exhausted while the main subscription may still have headroom.
    # Both recover the same way as other long_context_tier paths: cap at the
    # standard context tier and compress, then retry (see run_agent.py).
    if status_code in (400, 429) and "extra usage" in error_msg:
        _long_ctx_gate = status_code == 429 and "long context" in error_msg
        _meter_exhausted = (
            "out of extra usage" in error_msg
            or "settings/usage" in error_msg  # e.g. claude.ai/settings/usage
        )
        if _long_ctx_gate or _meter_exhausted:
            return _result(
