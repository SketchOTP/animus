...
# Native Anthropic model used when auto-appending Claude after Codex exhaustion.
# Resolved credentials may be ANTHROPIC_API_KEY or Claude Code OAuth (see
# ``resolve_anthropic_token``).
_CODEX_DEFAULT_CLAUDE_SUBSCRIPTION_FALLBACK_MODEL = "claude-sonnet-4-6"


def _augment_fallback_chain_with_claude_subscription(
    chain: List[Dict[str, Any]],
    *,
    provider: str,
) -> List[Dict[str, Any]]:
    """Append Anthropic native fallback when OpenAI Codex is the primary provider.

    Codex (ChatGPT OAuth) hits subscription and rate limits often. If the user
    has Anthropic access (API key or Claude Code credentials), append a native
    Anthropic target so ``_try_activate_fallback()`` can continue the session.

    Disabled when ``HERMES_DISABLE_CODEX_CLAUDE_FALLBACK`` is truthy, or when the
    chain already includes an Anthropic-class provider entry, or when no
    Anthropic token is available.
    """
    if env_var_enabled("HERMES_DISABLE_CODEX_CLAUDE_FALLBACK"):
        return chain
    ...
    for entry in chain:
        if isinstance(entry, dict) and _normalize_aux_provider(entry.get("provider")) == "anthropic":
            return chain
    try:
        from agent.anthropic_adapter import resolve_anthropic_token
    except Exception:
        return chain
    if not resolve_anthropic_token():
        return chain
    out = list(chain)
    out.append(
        {
            "provider": "anthropic",
            "model": _CODEX_DEFAULT_CLAUDE_SUBSCRIPTION_FALLBACK_MODEL,
        }
    )
    return out
...
class AIAgent:
    ...
    def __init__(
        ...
        if (
            ...
            and self.provider != "cursor-agent"
            and not str(self.base_url or "").lower().startswith("acp://copilot")
            and not str(self.base_url or "").lower().startswith("cursor-agent://")
        ...
        if isinstance(fallback_model, list):
            ...
            self._fallback_chain = []
        self._fallback_chain = _augment_fallback_chain_with_claude_subscription(
            self._fallback_chain,
            provider=self.provider,
        )
        self._fallback_index = 0
        self._fallback_activated = False
        self._auto_model_swap_snapshot = None
        self._auto_model_execution_model = None
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        if self.provider == "copilot-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://copilot"):
            ...
            return client
        if self.provider == "cursor-agent" or str(client_kwargs.get("base_url", "")).startswith(
            "cursor-agent://"
        ):
            ...
            safe_kwargs = {
                ...
                if k in {"api_key", "base_url", "default_headers", "command", "args", "timeout"}
            }
            client = CursorAgentClient(**safe_kwargs)
            logger.info(
                "Cursor Agent CLI client created (%s, shared=%s) %s",
                reason,
                shared,
                self._client_log_context(),
            )
    ...
    def _interruptible_streaming_api_call(
        ...
        # Subprocess-backed OpenAI shims (Copilot ACP, Cursor CLI) return a
        # full response object — they do not implement SSE chunk iteration.
        if self.provider in ("copilot-acp", "cursor-agent") or str(
            getattr(self, "base_url", "") or ""
        ).lower().startswith(("acp://copilot", "cursor-agent://")):
            return self._interruptible_api_call(api_kwargs)
    ...
    def _run_conversation_early_return(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Restore per-turn auto model before returning from the tool loop."""
        self._auto_model_turn_end()
        return payload

    def _auto_model_turn_begin(self, user_message: str, history_len: int) -> None:
        """Optionally swap ``self.model`` for this turn (same provider/credentials)."""
        self._auto_model_swap_snapshot = None
        self._auto_model_execution_model = None
        if getattr(self, "_delegate_depth", 0):
            return
        ...
        try:
            ...
            ts_len = len(self.enabled_toolsets) if self.enabled_toolsets else 12
            codex_token = None
            if (self.provider or "").strip().lower() == "openai-codex":
                ak = getattr(self, "api_key", None)
                codex_token = ak.strip() if isinstance(ak, str) and ak.strip() else None
            selected, _complexity = select_model_for_turn(
                provider=self.provider or "",
                baseline_model=self.model or "",
                user_message=user_message,
                history_message_count=history_len,
                enabled_toolset_count=ts_len,
                agent_cfg=agent_cfg,
                codex_access_token=codex_token,
            )
        except Exception as exc:
            logger.debug("auto_model_by_complexity begin skipped: %s", exc)
            return
        if not selected or selected == (self.model or ""):
            return
        ...
        snap: Dict[str, Any] = {
            ...
            "use_prompt_caching": self._use_prompt_caching,
            "use_native_cache_layout": self._use_native_cache_layout,
            "cc_model": getattr(_cc, "model", self.model) if _cc else self.model,
            "cc_context_length": _cc.context_length if _cc else 0,
            "cc_base_url": getattr(_cc, "base_url", self.base_url) if _cc else self.base_url,
            "cc_api_key": getattr(_cc, "api_key", "") if _cc else "",
            "cc_provider": getattr(_cc, "provider", self.provider) if _cc else self.provider,
            "cc_api_mode": self.api_mode,
        }
        self._auto_model_swap_snapshot = snap
        self.model = selected
        self._auto_model_execution_model = selected
        if hasattr(self, "_transport_cache"):
            self._transport_cache.clear()
        self._use_prompt_caching, self._use_native_cache_layout = (
            self._anthropic_prompt_cache_policy(
        ...
        if _cc:
            try:
                ...
                logger.debug("auto_model_by_complexity compressor update failed: %s", exc)

    def _auto_model_turn_end(self) -> None:
        """Restore runtime fields after a per-turn auto model swap."""
        self._auto_model_execution_model = None
        snap = getattr(self, "_auto_model_swap_snapshot", None)
        if not snap:
            self._auto_model_swap_snapshot = None
            return
        self.model = snap["model"]
        self._use_prompt_caching = snap["use_prompt_caching"]
        self._use_native_cache_layout = snap["use_native_cache_layout"]
        _cc = getattr(self, "context_compressor", None)
        if _cc:
            try:
                _cc.update_model(
                    model=snap["cc_model"],
                    context_length=snap["cc_context_length"],
                    base_url=snap["cc_base_url"],
                    api_key=snap["cc_api_key"],
                    provider=snap["cc_provider"],
                    api_mode=snap.get("cc_api_mode", self.api_mode),
                )
            except Exception as exc:
                logger.debug("auto_model_by_complexity compressor restore failed: %s", exc)
        if hasattr(self, "_transport_cache"):
            self._transport_cache.clear()
        self._auto_model_swap_snapshot = None
    ...
    def run_conversation(
        ...
        # Clear any orphaned per-turn auto-model swap if a prior turn crashed
        # before the normal cleanup path ran.
        self._auto_model_turn_end()
        ...
        # Optional per-turn model swap (restored after this turn's tool loop).
        _um_for_complexity = (
            original_user_message
            if isinstance(original_user_message, str)
            else str(original_user_message or "")
        )
        self._auto_model_turn_begin(
            _um_for_complexity,
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
                ...
                if self.provider == "nous":
                    try:
                        ...
                        if _nous_remaining is not None and _nous_remaining > 0:
                            ...
                            return self._run_conversation_early_return({
                                "final_response": (
                                    f"⏳ {_nous_msg}\n\n"
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if fallback and getattr(self, '_last_content_tools_all_housekeeping', False):
                            ...
                            if (
                                ...
                                and final_response
                                and not self._interim_content_was_streamed(final_response)
                            ):
                                self._fire_stream_delta(final_response)
                            break
                    ...
                    # Subprocess shims (cursor-agent, copilot-acp) use non-streaming
                    # HTTP internally, so no token deltas reach stream_delta_callback
                    # during the call.  Gateway /v1/chat/completions SSE would stay
                    # empty (only role + [DONE]) without this flush.
                    _flush_visible = (final_msg.get("content") or "").strip()
                    if (
                        self._has_stream_consumers()
                        and _flush_visible
                        and not self._interim_content_was_streamed(_flush_visible)
                    ):
                        self._fire_stream_delta(_flush_visible)
        ...
        _turn_execution_model = getattr(self, "_auto_model_execution_model", None) or self.model
        self._auto_model_turn_end()
