...
# ─── URL Parsing Helpers ──────────────────────────────────────────────────────
# Prefer ``url_host_utils`` (single-file deploy / preflight patches).  Inline
# fallback keeps ``from utils import base_url_host_matches`` working if that
# module is missing from a partial checkout (Hermes Chat + gateway share utils).
try:
    from url_host_utils import base_url_host_matches, base_url_hostname  # noqa: F401
except ImportError:
