...
class TestClassifyApiError:
    ...
    def test_anthropic_400_out_of_extra_usage_meter(self):
        """HTTP 400 'out of extra usage' is the add-on meter, not main plan tokens."""
        msg = (
            "HTTP 400: You're out of extra usage. Add more at claude.ai/settings/usage "
            "and keep going."
        )
        e = MockAPIError(msg, status_code=400)
        result = classify_api_error(e, provider="anthropic", model="claude-opus-4.6")
        assert result.reason == FailoverReason.long_context_tier
        assert result.retryable is True
        assert result.should_compress is True
