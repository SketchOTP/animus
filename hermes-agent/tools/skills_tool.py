...
def _find_all_skills(*, skip_disabled: bool = False, include_paths: bool = False) -> List[Dict[str, Any]]:
    ...
    Args:
        ...
        include_paths: If True, each entry includes a ``path`` key (absolute
            ``SKILL.md`` path) for editors and tooling.
    ...
    for scan_dir in dirs_to_scan:
        for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
            ...
            try:
                ...
                if include_paths:
                    try:
                        entry["path"] = str(skill_md.resolve())
                    except OSError:
