...
class TestDelegationAcpDefaults(unittest.TestCase):
    """Claude ACP defaults + complexity-based model auto-selection."""

    @patch("tools.delegate_tool._load_config")
    @patch("run_agent.AIAgent")
    def test_defaults_to_claude_with_low_complexity_model(self, MockAgent, mock_cfg):
        mock_cfg.return_value = {
            "acp_auto_model_selection": True,
            "max_iterations": 50,
        }
        parent = _make_mock_parent()
        mock_child = MagicMock()
        ...
        self.assertIn("--acp", kwargs["acp_args"])
        self.assertIn("--stdio", kwargs["acp_args"])
        self.assertEqual(kwargs["acp_args"][-2:], ["--model", "haiku"])

    @patch("tools.delegate_tool._load_config")
    @patch("run_agent.AIAgent")
    def test_high_complexity_defaults_to_opus(self, MockAgent, mock_cfg):
        mock_cfg.return_value = {
            "acp_auto_model_selection": True,
            "max_iterations": 50,
        }
        ...
        _build_child_agent(
            task_index=0,
            goal=(
                "Design a comprehensive multi-file architecture migration with security, "
                "performance, and concurrency hardening"
            ),
            context="Perform deep review and root cause analysis before implementation.",
        ...
        kwargs = MockAgent.call_args.kwargs
        self.assertEqual(kwargs["acp_command"], "claude")
        self.assertEqual(kwargs["acp_args"][-2:], ["--model", "opus"])

    @patch("tools.delegate_tool._load_config")
    @patch("run_agent.AIAgent")
    def test_explicit_model_arg_is_respected(self, MockAgent, mock_cfg):
        mock_cfg.return_value = {
            "acp_auto_model_selection": True,
            "max_iterations": 50,
        }
        ...
        _build_child_agent(
            ...
            override_acp_args=["--acp", "--stdio", "--model", "sonnet"],
        )

        kwargs = MockAgent.call_args.kwargs
        self.assertEqual(kwargs["acp_args"], ["--acp", "--stdio", "--model", "sonnet"])
