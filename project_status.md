...
- **PWA / multi-device chats:** `index.html` init always applies **`GET /api/convs`** (removed wrong `serverConvs.length >= convs.length` gate); refetch on **`visibilitychange`** + **`pageshow` (bfcache)** so phone reflects desktop deletes; **`sw.js`** cache **`animus-v35`** (+ **projects sidebar drag order** survives refetch: **`mergeProjectLists`** prefers local order while **`_projectsOrderDirty`** / **`sessionStorage`** until **`POST /api/projects`** OK); prior **v34** UI: Cron **overseer**, **timezone `<dialog>`**, Plan **modal**, chat **ghost** layout, **default `general`** session auto-open).
