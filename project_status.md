...
- **Tokens tab:** **`mergeServerTokenEntriesFull`** + **`GET /api/tokens/usage?full=1`**; **`animus_client`** JSONL + CSV; **ANIMUS client breakdown** (`aggregateAnimusClientTotals` / **`renderAnimusClientBreakdown`**) — fixed rows **`web`…`prompt-optimizer`** + **Uncategorized / missing** (in/out/total/rows/% of total); **By source (logging)** tab copy distinguishes JSONL **`source`** vs product **`animus_client`**. PWA **`X-Animus-Client`** slugs per **`token_usage.ANIMUS_CLIENT_SLUGS`**; server stamps **help** / **cron** / **prompt-optimizer**; Cursor inference on **`/api/chat`** only when header absent. **`sw.js`** **`animus-v57`**, **`CHAT_SERVER_REV`** **v59** (`token-charts-collapsible`); token charts **collapsible** (`<details>`): **This month** + **By year** default **collapsed**, **By calendar month (all-time)** default **open**; **chart reflow** on expand. **Auto-refresh** still **10 min** while Tokens tab active + visible. **Verification:** **PASS** — backend contract + **`GET ?full=1`** + static **`index.html`** (CSV, **`ANIMUS:`** log, client breakdown DOM/helpers). **PARTIAL** — full browser E2E, downloaded CSV bytes, **`/api/chat`** SSE proof. Doc: **`artifacts/token_tracker_tab_explained.md`**.
- **Chat ↔ Hermes session:** **`GET /api/hermes/session-prompt-status`** preflight (gateway **`/v1/chat/session-prompt-status`**) before each project send when an assistant message exists — reconciles **`hermes_has_stored_system_prompt`** + **`hermes_stored_prompt_confirmed_session_id`** vs SQLite so stale local omit flags do not drop project context; **`buildMessages`** omits duplicate project system only when flags match **current** **`session_id`**; **`hermes.session`** SSE + non-stream headers; legacy gateway **404** skips preflight.
